...
/// </summary>
public sealed partial class RecentlyPlayed : SongListPageBase
{
	/// <summary>
	...
	/// <inheritdoc/>
	protected override ObservableCollection<Song> PageSongs => RecentlyPlayedSongs;

	/// <inheritdoc/>
	protected override string PlaylistKey => "RecentlyPlayed";
	...
	/// <inheritdoc/>
	protected override Page CreateFreshPage() => new RecentlyPlayed();
	...
	private async Task UpdateListBasedOnMaxLimit()
	{
		...
		if (RecentlyPlayedSongs.Count > 0)
		{
			...
			song = RecentlyPlayedSongs.Select(s => s).Where(s => s.Path == song?.Path).FirstOrDefault();
